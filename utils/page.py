...
class Pagination(object):
    ...
    def page_html_js(self):
        ...
        if self.current_page == 1:
            ...
            prev = ' <li><a href="#" url="%s" page="%s" >上一页</a></li>' % (self.base_url,self.current_page - 1,)
        page_list.append(prev)
        ...
        for i in range(pager_start, pager_end):
            if i == self.current_page:
                tpl = ' <li class="active"><a href="#" url="%s" page="%s" >%s</a></li>' % (self.base_url,i, i,)
            else:
                tpl = ' <li><a href="#" url="%s" page="%s" >%s</a></li>' % (self.base_url,i, i)
            page_list.append(tpl)

        if self.current_page >= self.max_pager_num:
            nex = ' <li class="disabled"><a href="#">下一页</a></li>'
        else:
            nex = ' <li><a href="#" url="%s" page="%s" >下一页</a></li>' % (self.base_url,self.current_page + 1,)
        page_list.append(nex)
